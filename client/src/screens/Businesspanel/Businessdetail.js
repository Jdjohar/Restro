...
const handleChangeFont = (selectedFont) => {
  ...
  const fetchOffers = async () => {
    try {
      const userid = localStorage.getItem('userid');
      const response = await fetch(`http://localhost:3001/api/offerbybusinessid?businessId=${businessId}`);
  ...
  const fetchweeklyOffers = async () => {
    try {
      const userid = localStorage.getItem('userid');
      const response = await fetch(`http://localhost:3001/api/weeklyofferbybusiness?businessId=${businessId}`);
      const data = await response.json();
