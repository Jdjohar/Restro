...
router.post('/saveData', async (req, res) => {
  try {
    const { jsonData,imageUrl, name, category } = req.body;
    console.log('Received data:', req.body);
    // Assuming your Template model has fields: jsonData, image, name, category
...
// router.get('/get-temp-child/:storeeid/:tempid', async (req, res) => {
//     const storeId = req.params.storeeid; 
//     console.log('storeId:->', storeId); 
//     const tempId = req.params.tempid;
//     const products = await Product.find({ storeId, tempId  });
//     console.log(products, 'products');
//     const productsCount = products.length;
//     const template = await TemplateSchema.findOne({ storeId, _id: tempId });

//         if (!template) {
//             return res.status(404).json({ message: 'Template not found for the given store ID and template ID' });
//         }
//         const children = [

//         ]

//     // Define constants for page width, height
//     const PAGE_WIDTH = 595.2755905511812;
//     const PAGE_HEIGHT = 841.8897637795276;

//     // Calculate max children per page based on products count and treating 3 children as 1 unit
//     const MAX_CHILDREN_PER_PAGE = Math.ceil(productsCount / 3);
...
//         // Check if the maximum children per page is reached or if it's the last product
//         if (upchildrens.length >= MAX_CHILDREN_PER_PAGE * 12 || i === products.length - 1) {
//             // Create a new page if the maximum children per page is reached or it's the last product
//             pages.push({
//                 id: `page_${pageIndex}`,
...
//             upchildrens = []; // Reset children array for the new page
//             pageIndex++; // Increment page index
//             childindex = productIndex * 3; // Reset child index to the next product's children
//             productIndex++;
//         }
//     }

//     // Send response with pages data
//     res.json({
//         width: PAGE_WIDTH,
//         height: PAGE_HEIGHT,
//         fonts: [],
...
router.get('/get-temp-child/:storeeid/:tempid', async (req, res) => {
    try {
        ...
        // Fetch all templates based on store ID
        const template = await TemplateSchema.findById(tempId);
        ...
        // for (const template of templates) {
            // Parse children data from the template
            const childrenData = JSON.parse(template.jsonData);
        const PAGE_WIDTH = childrenData.width;
        const PAGE_HEIGHT = childrenData.height;
        const Fonts = childrenData.fonts;
        const Unit = childrenData.unit;
        const Dpi = childrenData.dpi;
        // const pagesIds = [];
        // if (childrenData && childrenData.pages) {
        //     // Loop through each page in childrenData
        //     childrenData.pages.forEach(page => {
        //         // Get the children array from the current page
        //         const pageid = page.id;
        //         console.log(pageid, 'pageid');
        //     });
        // }
            const children = [];
            let restpageChildren = [];

            // Check if childrenData and childrenData.pages are defined
            if (childrenData && childrenData.pages) {
                
                // Loop through each page in childrenData
                childrenData.pages.forEach(page => {
                    // Get the children array from the current page
                    const pageChildrenold = page.children;
let pageChildren = pageChildrenold.filter(child => child.type == 'text' && child.fontSize < 25);
restpageChildren = pageChildrenold.filter(child => child.type != 'text' || child.fontSize > 25);
                    // Check if pageChildren is defined
                    if (pageChildren) {
                        // Push each child from the current page into the children array
            ...
            // Calculate max children per page based on products count and treating 3 children as 1 unit
            const MAX_CHILDREN_PER_PAGE = Math.ceil( products.length / (children.length/3));
            console.log("vsdf",MAX_CHILDREN_PER_PAGE);

            const pages = [];
            let pageIndex = 1;
            let cholindex = 0;
            for (let x = 0; x <= MAX_CHILDREN_PER_PAGE; x++) {
            let upchildrens = [];
            
            restpageChildren.forEach(child => {
                upchildrens.push(child);
            });
            let childindex = 0;
            let productIndex = 0;
            for (let i = cholindex; i < products.length; i++) {
                // Check if children[childindex] is defined before accessing its properties
                if (children[childindex]) {
                    // Push child data from the template into upchildrens array
                    upchildrens.push({
                        ...children[childindex++],
                        text: products[i].name
                    });
                    ...
                    // // Check if the maximum children per page is reached or if it's the last product
                    // if (upchildrens.length >= (products.length * 3) || childindex === children.length) {
                    //     // Create a new page if the maximum children per page is reached or it's the last product
                    //     pages.push({
                    //         id: `page_${pageIndex}`,
                    //         children: upchildrens,
            ...
            upchildrens = []; // Reset children array for the new page
            x++; // Increment page index
            // allPages.push(...pages);
        }

            // Push pages for this template into allPages array
        // }

        // Send response with all pages data
        res.json({
            width: PAGE_WIDTH,
            height: PAGE_HEIGHT,
            fonts: Fonts,
        ...
        console.error('Error fetching template children:', error);
        res.status(500).json({ message: 'Error fetching template children' });
    }
});
...
//       // Fetch template based on store ID and template ID
//       const template = await TemplateSchema.findOne({ storeId, _id: tempId });
...
//       const childrenData = JSON.parse(template.description);
...
//       // Calculate max children per page based on products count and treating 3 children as 1 unit
//       const MAX_CHILDREN_PER_PAGE = Math.ceil(products.length / 3);
...
//             "alwaysOnTop": false,
//             "showInExport": true,
//             "x": 380.27559054118075,
//             "y": 40.538146714264684,
//             "width": 215,
//             "height": 37,
//             "rotation": 0,
//             "animations": [],
...
//     // if((childrenCount/3)/productsCount )
//     console.log('Number of productsCount:', productsCount);
//     console.log('Number of pages:', 4/(childrenCount/3));
...
//             });
//         }
//             if(children[childindex] != null && children[childindex] != undefined){
//             upchildrens.push({
//                 ...children[childindex++],
//                 text: products[i].description
//             });
...
//     //             x: 22.39468806800201, // Adjust X, Y positions as needed
...
// });
  ...
  router.get('/get-templates/', async (req, res) => {
    try {
        const storeeid = req.query.storeeid;

        // Assuming TemplateSchema is your Mongoose model for templates
        const templates = await TemplateSchema.find({ storeId: storeeid });

        if (templates.length === 0) {
            return res.status(404).json({ message: 'No templates found for the given store ID' });
        }

        console.log('Templates found:', templates.length);

        // const items = templates.map(template => ({
        //     json: `https://restroproject.onrender.com/api/get-temp-child/${storeeid}/${template._id}`,
        //     preview: "https://jdwebservices.com/img/ind.webp"
        // }));

        const items = await Promise.all(templates.map(async template => {
            const imageUrl = await getImageUrl(template._id); // Function to fetch imageUrl by template id
            return {
                json: `https://restroproject.onrender.com/api/get-temp-child/${storeeid}/${template._id}`,
                preview: imageUrl ? imageUrl : "https://jdwebservices.com/img/ind.webp"
            };
        }));

        const temppolotno = {
            hits: templates.length,
            totalPages: Math.ceil(templates.length / 10), // Assuming 10 templates per page
            items: items,
        };
...
async function getImageUrl(templateId) {
    try {
  ...
  //   // Assuming TemplateSchema is your Mongoose model for templates
  //   const template = await TemplateSchema.findOne({ storeId: storeeid });

  //   if (!template) {
  //       return res.status(404).json({ message: 'Template not found for the given store ID' });
  //   }

  //   const tempid = template._id;
  ...
  //       "items": [
  //         {
  //           "json": `https://restroproject.onrender.com/api//get-temp-child-1/${storeeid}`,
  //           "preview": "https://jdwebservices.com/img/ind.webp"
  //         },
  //         {
  //           "json": `https://restroproject.onrender.com/api/get-temp-child/${storeeid}/${tempid}`,
  //           "preview": "https://jdwebservices.com/img/ind.webp"
  //         },
